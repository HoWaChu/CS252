...
class Transformation(analysis.Analysis):
    ...
    def translate(self, magnitudes):
        ...
        result = self.translation_matrix(magnitudes) @ h_data.T
        result = result.T[:, :-1]
    ...
    def scale(self, magnitudes):
        ...
        '''
        h_data = self.get_data_homogeneous()
        result = self.scale_matrix(magnitudes) @ h_data.T
        result = result.T[:, :-1]
        self.data = data.Data(data = result, headers = self.data.headers, header2col= self.data.header2col)
        return self.data.get_all_data()
    ...
    def normalize_together(self):
        ...
        for i in range(len(self.data.get_headers())):
            t_magnitude_list.append(-(self.data.get_all_data().min()))
            s_magnitude_list.append(1/(self.data.get_all_data().max()) - self.data.get_all_data().min())
    ...
    def normalize_separately(self):
        ...
        t_magnitude_list = []
        s_magnitude_list = []
        min = self.min(self.data.get_headers()) # returns a list of mins per variable
        max = self.max(self.data.get_headers()) # returns a list of maxs per variable
        for i in range(len(self.data.get_headers())):
            t_magnitude_list.append(-(min[i]))
            s_magnitude_list.append(1/(max[i] - min[i]))
        C = self.scale_matrix(s_magnitude_list) @ self.translation_matrix(t_magnitude_list)
        return self.transform(C)
