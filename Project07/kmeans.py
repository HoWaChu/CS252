...
class KMeans:
    ...
    def initialize_plusplus(self, k):
        ...
        - To pick the i-th centroid (i > 0)
            ...
            distribution.
        '''
        self.centroids = np.zeros([k, self.num_features])
        self.centroids[0, :] = self.data[np.random.randint(self.num_samps), :]
        min_dist = np.zeros([self.num_samps,])
        ...
        for j in range(1, k):
            ...
            prob = min_dist**2
            prob /= np.sum(prob)
            a = np.random.choice(self.num_samps, p = prob)
            self.centroids[j, :] = self.data[a, :]
        
        return self.centroids

    def cluster(self, k=2, tol=1e-2, max_iter=1000, verbose=False, init_function = 'random'):
        ...
        diff = float('inf')
        while iter <= max_iter and np.abs(np.mean(diff)) >= tol:
        ...
        if verbose:
            print(f"Total number of iterations K-means ran for: {iter}")
    ...
    def cluster_batch(self, k=2, n_iter=1, verbose=False, init_method = 'random'):
        ...
        verbose: boolean. Print out debug information if set to True.
        init_method: str. The kmeans initialization method. 'random or kmeans++'
        '''
        min_inertia = []
        centroids = []
        data_labels = []
        ...
        for i in range(n_iter):
            inertia, iter = self.cluster(k, max_iter = n_iter, verbose = verbose, init_function = init_method)
            min_inertia.append(inertia)
            centroids.append(self.centroids)
            data_labels.append(self.data_centroid_labels)
            count.append(iter)

        min_index = np.argmin(min_inertia)

        self.centroids = centroids[min_index]
        self.data_centroid_labels = data_labels[min_index]
        self.inertia = min_inertia[min_index]
    ...
    def update_labels(self, centroids):
        ...
        for i in range(self.data.shape[0]):
            temp = []
            for j in centroids:
                temp.append(self.dist_pt_to_pt(self.data[i,:], j))
    ...
    def update_centroids(self, k, data_centroid_labels, prev_centroids):
        ...
        for i in range(k):
            x_ind = np.where(data_centroid_labels == i)[0]   
            if len(x_ind) > 0:
                new_centroids[i,:] = np.mean(self.data[x_ind, :], axis = 0)
            else:
                new_centroids[i, :] = self.data[np.random.randint(self.num_samps), :]
    ...
    def compute_inertia(self):
        ...
        for i in range(self.data.shape[0]):
            ...
            sum += (self.dist_pt_to_pt(self.data[i, :], self.centroids[int(index), :]))**2
    ...
    def plot_clusters(self):
        ...
        cluster_colors = [colors[int(label)] for label in self.data_centroid_labels]

        plt.scatter(self.data[:, 0], self.data[:, 1], c=cluster_colors)

        plt.scatter(self.centroids[:, 0], self.centroids[:, 1], c='black', marker='x', s=100)
