...
class analysis_Ext:
    ...
    def scatter(self, ind_var, dep_var, title):
        ...
        y = y.astype(float)
        plt.scatter(x, y)
        plt.xticks(['1/3/2020', '6/5/2020','6/17/2021', '2/2/2022', '2/10/2023'], rotation='vertical')
        plt.xlabel(ind_var)
        plt.ylabel(dep_var)
        plt.title(title)
        return x, y
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        fig, axes = plt.subplots(len(data_vars), len(data_vars), sharex = True, sharey = True)
        ...
        for i, x_var in enumerate(data_vars):
            for j, y_var in enumerate(data_vars):
                ax = axes[i][j]
                if i == j:
                    ax.hist(self.data.select_data(headers = [x_var]))
                    ax.set_xlabel(x_var)
                    ax.set_ylabel(y_var)
                else:
